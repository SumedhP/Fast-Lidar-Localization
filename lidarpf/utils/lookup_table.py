...
@njit
def _bresenhams_ray_cast(start_x: int, start_y: int, theta: float, occupancy_grid: npt.NDArray[np.bool_]) -> float:
    """
    Perform Bresenham's ray casting algorithm to find the distance to the nearest obstacle from a given start point.
    """
    assert occupancy_grid.ndim == 2, "Occupancy grid must be a 2D array."
    if (start_x < 0 or start_y < 0 or start_x >= occupancy_grid.shape[ParticleState.X] or start_y >= occupancy_grid.shape[ParticleState.Y]):
        return 0
    dx = np.cos(theta)
    dy = np.sin(theta)
    x, y = start_x, start_y
...
@njit(parallel=True)
def compute_lookup_table(occupancy_grid: npt.NDArray[np.bool_], num_angles: int) -> npt.NDArray[np.float64]:
    """
